...
# -*- coding: utf-8 -*-
"""
Script lấy số điện từng ngày từ EVN Việt Nam
...
class EVNSimple:
    ...
    def login_evnhcmc_multi_domain(self, username, password):
        ...
        for base_url in self.evnhcmc_domains:
            print(f"🔗 Thử kết nối EVNHCMC tới: {base_url}")
            
            if not self.test_connection(base_url):
                print(f"❌ Không thể kết nối tới {base_url}")
                continue
                
            print(f"✅ Kết nối thành công tới {base_url}")
            
            for endpoint in login_endpoints:
                login_url = f"{base_url}{endpoint}"
                print(f"🔐 Thử đăng nhập EVNHCMC: {login_url}")
                ...
                for payload in payloads:
                    try:
                        ...
                        if response.status_code == 200:
                            try:
                                ...
                                if "access_token" in data or "token" in data or "jwt" in data:
                                    ...
                                    print(f"✅ Đăng nhập EVNHCMC thành công! Token: {token[:20]}...")
                                    return base_url
                            except:
                                pass
                        ...
                        print(f"⚠️ Lỗi với {login_url}: {str(e)[:100]}")
                        continue
        
        print("❌ Không thể đăng nhập EVNHCMC!")
        return None
    ...
    def login_evnhanoi_multi_domain(self, username, password):
        ...
        for base_url in self.evnhanoi_domains:
            print(f"🔗 Thử kết nối EVNHANOI tới: {base_url}")
            ...
            for endpoint in login_endpoints:
                login_url = f"{base_url}{endpoint}"
                print(f"🔐 Thử đăng nhập EVNHANOI: {login_url}")
                        ...
                        if response.status_code == 200:
                            try:
                                ...
                                if "access_token" in data:
                                    ...
                                    print(f"✅ Đăng nhập EVNHANOI thành công! Token: {self.access_token[:20]}...")
                                    return base_url
                            except:
                                pass
        ...
        print("❌ Không thể đăng nhập EVNHANOI!")
        return None
    ...
    def create_mock_data(self):
        ...
        result = {
            ...
            "customer_info": {
                "ten_khachhang": "Khách hàng TPHCM Demo",
            ...
            "note": "Đây là dữ liệu giả vì không thể kết nối tới EVN!"
        }
    ...
    def get_daily_consumption(self, username, password, customer_id):
        ...
        print(f"🔍 Phát hiện khu vực: {region} (User ở HCM)")
        ...
        if region == 'EVNHCMC':
            ...
            if base_url:
                ...
                print("⚠️ EVNHCMC không được, thử EVNHANOI...")
                ...
                if base_url:
                    ...
                    print("⚠️ Không thể đăng nhập EVN, tạo dữ liệu demo...")
            ...
            print(f"❌ Chưa hỗ trợ khu vực {region}. Tạo dữ liệu demo...")
            return self.create_mock_data()
    ...
    def get_daily_consumption_evnhcmc(self, base_url, customer_id):
        ...
        for info_endpoint in info_endpoints:
            for consumption_endpoint in consumption_endpoints:
                try:
                    print(f"🔍 Thử API EVNHCMC: {base_url}{info_endpoint}")
                    ...
                    print(f"⚠️ Lỗi API EVNHCMC {info_endpoint}: {str(e)[:100]}")
                    continue
        
        print("❌ Không thể lấy dữ liệu từ bất kỳ API EVNHCMC nào, tạo dữ liệu demo...")
        return self.create_mock_data()
    ...
    def get_daily_consumption_evnhanoi(self, base_url, customer_id):
        ...
        for info_endpoint in info_endpoints:
            for consumption_endpoint in consumption_endpoints:
                try:
                    print(f"🔍 Thử API EVNHANOI: {base_url}{info_endpoint}")
                    ...
                    print(f"⚠️ Lỗi API EVNHANOI {info_endpoint}: {str(e)[:100]}")
                    continue
        
        print("❌ Không thể lấy dữ liệu từ bất kỳ API EVNHANOI nào, tạo dữ liệu demo...")
        return self.create_mock_data()
...
def main():
    ...
    if data:
        ...
        print(f"📍 Khu vực: {data.get('region', 'N/A')}")
        print(f"👤 Tên khách hàng: {customer_info.get('ten_khachhang', 'N/A')}")
        print(f"📍 Địa chỉ: {customer_info.get('dia_chi', 'N/A')}")
        print(f"⚡ Mã công tơ: {customer_info.get('ma_congto', 'N/A')}")
        
        if isinstance(consumption, dict):
            if 'data' in consumption:
                daily_data = consumption['data']
                if isinstance(daily_data, list) and len(daily_data) > 0:
                    print(f"\n📈 Dữ liệu {len(daily_data)} ngày gần nhất:")
            ...
            if 'tong_san_luong_thang' in consumption:
                print(f"\n📊 Tổng sản lượng: {consumption['tong_san_luong_thang']} kWh")
                print(f"💰 Tổng tiền ước tính: {consumption['tong_tien_uoc_tinh']:,.0f} VNĐ")
        
        return True
    else:
        print("❌ Không thể lấy dữ liệu số điện!")
        return False
