...
export const firestoreService = {
  ...
  async getAllTransactions(familyId: string): Promise<Transaction[]> {
    try {
      ...
      // Sort client-side theo thời gian tạo chính xác
      const sortedTransactions = transactions.sort(sortTransactionsByCreationTime);
  ...
  async getStickyNotes(familyId: string): Promise<StickyNote[]> {
    try {
      ...
      // Sort by updatedAt, pinned notes first
  ...
  // Bulk operations for sticky notes
  async bulkDeleteStickyNotes(noteIds: string[]): Promise<void> {
    try {
      console.log('🔄 [firestoreService.bulkDeleteStickyNotes] Bulk deleting sticky notes:', noteIds);
      
      if (noteIds.length === 0) {
        console.log('⚠️ No sticky note IDs provided, skipping delete');
        return;
      }
      ...
      noteIds.forEach((id, index) => {
        console.log(`📝 Adding delete operation ${index + 1}/${noteIds.length} for sticky note ID: ${id}`);
        const docRef = doc(db, 'sticky_notes', id);
        batch.delete(docRef);
      });
      
      console.log('💾 Committing batch delete operation for sticky notes...');
      await batch.commit();
      console.log('✅ Batch delete committed successfully for sticky notes');
